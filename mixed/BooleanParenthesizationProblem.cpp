...
int countWays(string str, int i, int j, int isTrue){
    ...
    for(int k=i+1;k<=j-1;k=k+2){
        ...
        if(str[k]=='^'){
            if(isTrue){
                result += (leftT*rightF) + (leftF*rightT);
            } else {
                result += (leftT*rightT) + (leftF*rightF);
            }
        } else if(str[k]=='&'){
            if(isTrue){
                result += (leftT*rightT);
            } else {
                result += (leftT*rightF) + (leftF*rightT) + (leftF*rightF);
            }
        } else if(str[k]=='|'){
            if(isTrue){
                result += (leftT*rightF) + (leftF*rightT) + (leftT*rightT);
            } else {
                result += (leftF*rightF);
            }
        }
