...
int getRangeSumUtil(int* sTree, int aBeg, int aEnd, int qBeg, int qEnd, int idx){
    ...
    int mid = (aBeg+aEnd) >> 1;
    return getRangeSumUtil(sTree,aBeg,mid,qBeg,qEnd,2*idx+1) + getRangeSumUtil(sTree,mid+1,aEnd,qBeg,qEnd,2*idx+2);
}

int getRangeSum(int* sTree, int n, int qBeg, int qEnd){
    if(qBeg>qEnd || qBeg<0 || qEnd>=n){
        return -1;
    }
    return getRangeSumUtil(sTree, 0, n-1, qBeg, qEnd, 0);
}
...
int createSegmentTreeUtil(int arr[], int aBeg, int aEnd, int* sTree, int idx){
    if(aBeg == aEnd){
        ...
        int mid = (aBeg+aEnd) >> 1;
        sTree[idx] = createSegmentTreeUtil(arr,aBeg,mid,sTree,2*idx+1) + createSegmentTreeUtil(arr,mid+1,aEnd,sTree,2*idx+2);
    }
    return sTree[idx];
}
...
int main(){
    ...
    cout<<"ST SUM [3-5] = "<<getRangeSum(sTree,n,1,3)<<endl;
